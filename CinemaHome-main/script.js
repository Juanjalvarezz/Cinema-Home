function buscarPelis(query) {
          ...
          html += `
            <li>
              ...
              <div class="movie-info">
                ...
                <button class="view-more" data-movie-id="${movie.id}">Ver más</button>
              </div>
            </li>
          `;
        });
      ...
      success: function(data) {
        ...
        let html = `
          <div class="movie-detail">
            ...
            <h2>${data.title}</h2>
            <p>${data.overview}</p>
            <p>Género: ${data.genres.map(g => g.name).join(", ")}</p>
        ...
        videos.forEach(video => {
          html += `<li><a href="https://www.youtube.com/watch?v=${video.key}" target="_blank">${video.name}</a></li>`;
        });
        html += `
            </ul>
          </div>
  ...
  $("#search-form").submit(function(event) { 
    ...
    let query = $("#query").val().trim().length;
    if (!query) {
      buscarPelis(query);

      searchInput.addEventListener("input", debounce(() => {
        if (!searchInput.value.trim().length) return alert("Por favor escriba la pelicula")
